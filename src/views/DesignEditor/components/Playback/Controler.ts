...
// tao cac button de thay doi animation luon trong file nay?
...
class PlaybackController {
  ...
  private animate: Map<string, any>
  ...
  public play = () => {
    for (let [key, value] of this.resources) {
      ...
      this.applyImageAnimation(value.sprite, {
        repeat: true,
        reverse: true,
      })
  ...
  public applyImageAnimation = (sprite: PIXI.Sprite, animation: Animates) => {
    // function box(words:string, change){

    // }
    // const container = this.app.stage.addChild(new PIXI.Container())

    const box = IMAGE_ANIMATES.map((animate) => {
